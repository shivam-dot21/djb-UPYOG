  ...
  useEffect(async () => {
    if (preApprovedResponse?.data !== undefined) {
      const fileStoreIds = preApprovedResponse?.data.flatMap((item) =>
        item.documents.filter((doc) => doc?.additionalDetails?.fileName?.includes(".jpg")).map((doc) => doc?.fileStoreId)
      );
        ...
        <div style={{ marginTop: "10px", display: "flex", justifyContent: "space-between" }}>
          <div style={{ flex: 1 }}>
            ...
            <Dropdown
              ...
              optionKey="key"
              isMandatory={true}
              option={projectComponentOptions}
              selected={projectComponent}
              select={setProjectComponent}
            />
            ...
            {error === "" && (
              <React.Fragment>
                ...
                <div style={{ marginTop: "10px", marginBottom: "10px" }}>
                  <SubmitBar
                    label={t("SEARCH")}
                    onSubmit={getPreApprovedPlanDetails}
                    disabled={!lengthInFeet || !widthInFeet || !abuttingRoadWidth}
                  />
                </div>
              </React.Fragment>
            )}
            ...
            {estimateResponse?.data?.Calculations ? getDetailsRow(estimateResponse?.data?.Calculations[0]) : null}
          ...
          <div style={{ marginLeft: "20px", display: "flex", flexDirection: "column", alignItems: "flex-end", marginTop: "40px" }}>
            <img src={plotImage} alt="Description" style={{ width: "300px", height: "auto", borderRadius: "8px" }} />
          </div>
        </div>
      </FormStep>
