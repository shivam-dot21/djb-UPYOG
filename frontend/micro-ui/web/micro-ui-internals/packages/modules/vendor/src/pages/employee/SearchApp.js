...
import { TextInput, Label, SubmitBar, LinkLabel, ActionBar, CloseSvg, DatePicker, CardLabelError, SearchForm, SearchField, Dropdown, Toast } from "@djb25/digit-ui-react-components";
...
const SearchApp = ({path}) => {
    ...
    function onSubmit (_data) {
        ...
        setShowToast({ warning: true, label: "ERR_VENDOR_FILL_VALID_FIELDS" });
        else if(Object.entries(payload).length>0 && (payload.creationReason || payload.status ) && (!payload.requestId && !payload.fromDate && !payload.mobileNumber && !payload.requestId && !payload.toDate))
    ...
    return <React.Fragment>
        <VendorSearchApplication t={t} isLoading={isLoading} tenantId={tenantId} setShowToast={setShowToast} onSubmit={onSubmit} data={  isSuccess && !isLoading ? (searchReult.length>0? searchReult : { display: "ES_COMMON_NO_DATA" } ):""} count={count} /> 
        {showToast && (
        <Toast
