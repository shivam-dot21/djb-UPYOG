...
import { TickMark } from "../atoms/svgindex";
    ...
    <div
      ...
      style={
        isMobile
          ? {}
          : user?.type === "EMPLOYEE"
          ? { maxWidth: "960px", minWidth: "640px", margin: "0 auto", display: "flex", justifyContent: "center" }
    ...
    >
      {stepGroups.map((stepObj, index, arr) => (
        <div className="timeline-checkpoint" key={index}>
          ...
          {index < arr.length - 1 && <span className={`line ${stepObj.step < currentRouteStep ? "active" : ""}`}></span>}
        </div>
      ))}
    </div>
  );
